...
class RNNLM(object):
    ...
    def build(self):
        ...
        output_layer = softmax(self.n_hidden, self.E_table_size, h_r, h_c)
        ...
        self.train = theano.function(inputs=[self.x, self.x_mask_r, self.x_mask_c, self.y, self.y_mask, self.n_batch, lr],
                                     outputs=cost,
                                     updates=updates,
        ...
        self.test = theano.function(inputs=[self.x, self.x_mask_r, self.x_mask_c, self.y, self.y_mask, self.n_batch],
                                    outputs=[cost, output_layer.predict_r, output_layer.predict_c],
                                    givens={self.is_train: np.cast['int32'](0)})
