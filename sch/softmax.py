...
class softmax(object):
    def __init__(self, n_input, cluster_num, in_cluster_num, x_r, x_c):
        self.n_input = n_input
        self.cluster_num = cluster_num
        self.in_cluster_num = in_cluster_num
        ...
        init_Wr = np.asarray(np.random.uniform(low=-np.sqrt(1./self.n_input),
                                               ...
                                               size=(self.n_input, self.cluster_num)),
                             dtype=floatX)
        init_Wc = np.asarray(np.random.uniform(low=-np.sqrt(1./self.n_input),
                                               high=np.sqrt(1./self.n_input),
                                               size=(self.n_input, self.in_cluster_num)),
                             dtype=floatX)
        init_br = np.zeros((cluster_num, ), dtype=floatX)
        init_bc = np.zeros((in_cluster_num, ), dtype=floatX)
